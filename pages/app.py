...
@st.cache_resource    ## ...need to cache to prevent resource overload
def load_model():
    ## load model local
...
st.title("Not my first streamlit app")
...
fish_class_queue: "queue.Queue[]" = queue.Queue()
fish = ""
...
def videoFilter(frame: av.VideoFrame) -> av.VideoFrame:
    ...
    # text to print - get fish class from queue
    # fish_class = fish_class_queue.get()  ## - fish class queue
    text =  "HELLO! " + fish   
    cv2.putText(img, 
                text,
...
if st.checkbox("Show logits", value=True):
    if webrtc_ctx.state.playing:
        ...
        while True:
            ...
            # fish_class_queue.put(fish_class)  ## - fish class queue
            fish = fish_class
